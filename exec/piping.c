...
/*   Updated: 2024/05/31 13:34:27 by grebrune         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		find_cmd(t_head *head, t_cmd *copy, t_pipe *pipe)
{
	if (no_fork_cmd(head, copy, head->cmd->arg[0]) == 0)
		return (0);
	pipe->pid[pipe->x] = fork();
	if (pipe->pid[pipe->x] == 0)
	{
		redir_with_fd(pipe->fd, copy, pipe->x);
		dup_of_fd(pipe->fd[pipe->x], copy);
		if (ft_strcmp(copy->arg[0], "echo") == 0)
			return (ft_echo(head, copy, pipe->fd[pipe->x]), 1);
		if (ft_strcmp(copy->arg[0], "env") == 0)
			return (ft_env(head), 1);
		if (ft_strcmp(copy->arg[0], "pwd") == 0)
			return (ft_pwd(pipe->fd[pipe->x]), 1);
...
int		executable(t_head *head)
{
	...
	pipe->x = 0;
	pipe->pid = malloc(sizeof(int) * cmdlen(head->cmd));
	pipe->fd = malloc(sizeof(int) * cmdlen(head->cmd) - 1);
	open_the_pipe(pipe->fd, head);
	copy = head->cmd;
	while (copy)
	{
		find_cmd(head, copy, pipe);
		copy = copy->next;
		pipe->x++;
	}
	wait_for_all(pipe->pid, pipe->x);
//	free_pipe(pipe);
	return (0);
}
